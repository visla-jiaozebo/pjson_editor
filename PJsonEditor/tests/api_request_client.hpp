...
public:
    api_request_client() {
        ...
        if (std::filesystem::exists("/tmp/TOKEN_FILE")) {
            ...
            std::filesystem::file_time_type ftime = std::filesystem::last_write_time("/tmp/TOKEN_FILE");
            ...
            if (seconds.count() < 3600) { // within 1 hour
                ...
                ifs.close();
                std::cout << "Using cached auth token from /tmp/TOKEN_FILE" << std::endl;
            }
        }
    }
    ...
    nlohmann::json post(const std::string& endpoint, const nlohmann::json& request_body, 
